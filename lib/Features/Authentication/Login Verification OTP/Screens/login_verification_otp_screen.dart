...
class LoginOtpVerificationScreen extends ConsumerStatefulWidget {
  ...
  /// Merchant ID received from login API.
  final int merchantId;

  /// OTP expiry received from login API.
  ///
  /// Currently kept for future OTP expiry handling.
...
class _LoginOtpVerificationScreenState
  ...
  /// Resend button will remain disabled for 30 seconds.
  ...
  Future<void> _handleVerify() async {
    ...
    if (!RegExp(r'^\d{6}$').hasMatch(otp)) {
      _showError('OTP must contain numbers only.');
      return;
    }
    ...
    try {
      ...
      // OTP SUCCESS
      //
      // Token has already been saved inside OtpController.
      // ============================================================
  ...
  Future<void> _handleResend() async {
    ...
    try {
      ...
      // Start a fresh 30 second cooldown.
      _startResendTimer();
